...
#define INDEX_TO_QID(i) p->user_queue[i].queue_index,qos_hal_queue_track[p->user_queue[i].queue_index].queue_id
int qos_hal_dump_all_subiflist(uint32_t qosport)
{
	...
	for (p = port_subif_index->head; p; p = p->next) {
		if (p != NULL) {
			if (p->netdev != NULL) {
				pr_info("subif:0x%p atomic count:%d Ifname:[%s] queue_count:%d qos_port_idx:%d Qids:%d[%d],%d[%d],%d[%d],%d[%d],%d[%d],%d[%d],%d[%d],%d[%d],%d[%d],%d[%d],%d[%d],%d[%d],%d[%d],%d[%d],%d[%d],%d[%d]\n",
					p, atomic_read(&p->count), p->netdev?p->netdev->name:"NULL", p->queue_cnt, p->qos_port_idx,
					INDEX_TO_QID(0),
					INDEX_TO_QID(1),
					INDEX_TO_QID(2),
					INDEX_TO_QID(3),
...
qos_hal_ll_get_qos_mib(
		...
		uint32_t flag)
{
	struct dp_qos_queue_info inf;

	inf.nodeid = queueid;
	inf.inst = 0;

	if (dp_qos_get_q_mib(&inf, flag) == DP_FAILURE) {
		QOS_HAL_DEBUG_MSG(QOS_DEBUG_ERR, "<%s> dp_qos_get_q_mib failed\n", __func__);
		return QOS_HAL_STATUS_ERR;
	}
	QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "<%s> nodeid=%d, qocc=%d, qacc=%d, qdrp=%d\n",
				__func__,
				inf.nodeid, inf.qocc, inf.qacc, inf.qdrp);
	qos_mib->enqPkts = inf.qacc;
	qos_mib->deqPkts = inf.qacc - inf.qocc;
	qos_mib->dropPkts = inf.qdrp;
	return QOS_HAL_STATUS_OK;
}
	...
	for (m = 0; m <= 25; m++) {
		for (n = 0; n < QOS_HAL_MAX_SUB_IFID_PER_PORT; n++) {
			...
			if (strncmp(subif_index->dp_subif.ifname, dev_name, 16) == 0) {
				...
				dp_subif->subif  = subif_index->dp_subif.subif << port_prop.vap_offset;
				return QOS_HAL_STATUS_OK;
			}
		}
	}
		...
		int32_t flag)
{
	...
	if (no_of_entry == 0) {
		...
		q_map.qmap_entry = qos_mgr_malloc(sizeof(struct dp_queue_map_entry) * no_of_entry);
...
static int32_t qos_hal_lookup_subif_list_item_from_sched_input(uint32_t port,
			...
			struct qos_hal_user_subif_abstract **obj_head)
{
	...
	for (p = *obj_head; p; p = p->next) {
		if (p->port_sched_in_egress == port) {
			QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "<%s> port_sched_in_egress match found\n",
							__func__);
			if (obj) {
				*obj = p;
				ret = QOS_MGR_SUCCESS;
		...
		dp_subif_t *dp_subif)
{
	...
	if ((prio_level == subif_index->default_prio) && ((flags & QOS_MGR_Q_F_DEFAULT) == QOS_MGR_Q_F_DEFAULT)) {
		...
		if (dp_get_port_prop(0, subif_index->dp_subif.port_id, &port_prop)) {
			QOS_HAL_DEBUG_MSG(QOS_DEBUG_ERR, "dp_get_port_prop fail for dp_subif port id:%d\n", subif_index->dp_subif.port_id);
			return QOS_MGR_FAILURE;
		}
...
qos_hal_egress_queue_create(const struct qos_hal_equeue_create *param)
{
	...
	if ((param->scheduler_input >> 3) >= QOS_HAL_MAX_SCHEDULER) {
		...
		if (qos_hal_sched_track[idx].in_use == 0) {
			QOS_HAL_DEBUG_MSG(QOS_DEBUG_ERR, "%s:%d  Scheduler %d is not in use \n", __func__, __LINE__, idx);
			return QOS_HAL_STATUS_NOT_AVAILABLE;
		}
			...
			struct qos_hal_user_subif_abstract *subif_index)
{
	...
	if (eqcfg->codel_enable == 1) {
		q_param.codel_enable = 1;
	} else if (eqcfg->wred_enable == 1) {
...
qos_hal_scheduler_create(const struct qos_hal_sched_create *param)
{
	...
	if (qos_hal_sched_track[param->index].in_use) {
		QOS_HAL_DEBUG_MSG(QOS_DEBUG_ERR, "%s %d %d\n", __func__, __LINE__, param->index);
		return QOS_HAL_STATUS_NOT_AVAILABLE;
	}
	...
	QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "<%s> Scheduler:[%d] has priority:[%d] arbi:[%d] index %d\n", __func__, sch_id, prio_info.prio_wfq, prio_info.arbi, param->index);
#if 0
...
qos_hal_token_bucket_shaper_cfg_set(const struct qos_hal_token_bucket_shaper_cfg *param)
{
	...
	if (shaper_conf.type == DP_NODE_SCH) {
		shaper_conf.id.sch_id = qos_hal_sched_track[param->attach_to_id].id;
		 QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "<%s> shaper_conf.id.sch_id:%d\n", __func__, shaper_conf.id.sch_id);
	} else if (shaper_conf.type == DP_NODE_QUEUE) {
		shaper_conf.id.q_id = qos_hal_queue_track[param->attach_to_id].queue_id;
		 QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "<%s> shaper_conf.id.q_id:%d\n", __func__, shaper_conf.id.q_id);
	} else if (shaper_conf.type == DP_NODE_PORT) {
		shaper_conf.id.cqm_deq_port = param->attach_to_id;
		QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "<%s>  shaper_conf.id.cqm_deq_port:%d\n", __func__, shaper_conf.id.cqm_deq_port );
	} else {
		QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "<%s> Unknown type %d\n", __func__, param->attach_to);
		return QOS_HAL_STATUS_ERR;
	}
		...
		int32_t weight)
{
	...
	if (input_type == 0) /* Parent is Scheduler */
		sched_new.connected_scheduler_index = omid;
	else if (input_type == 1) /* Parent is Port */
		sched_new.port_idx = omid;
	else
		QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "Worng Input type [%d]\n", input_type);
	...
	if ((qos_hal_sched_track[omid >> 3].policy == QOS_HAL_POLICY_WFQ)
		...
		for (j = 0; j < 8; j++) {
			...
			if ((sie1 == DP_NODE_EN) && (sit1 == 1)) {
				...
				if (new_omid == QOS_HAL_STATUS_ERR) {
					QOS_HAL_DEBUG_MSG(QOS_DEBUG_ERR, "%s %d\n", __func__, __LINE__);
					return QOS_HAL_STATUS_ERR;
				}
		...
		if (sie == DP_NODE_EN) { /* input is enabled */
			if (sit == 0) { /* input is QID */
				...
				if ((temp_index->is_logical == 0) &&
					...
					/* Get subif node from list based on next port_sched_in_egress as current subif node dev is not present */
					if (qos_hal_lookup_subif_list_item_from_sched_input(
									(sched_input << 3 | (j+1)),
									&temp_index,
									&subif_index->head) == QOS_MGR_SUCCESS) {
						if (temp_index != NULL && temp_index->netdev->name[0]) {
							temp_index->port_sched_in_egress = (sched_input << 3 | j);
							QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "<%s>  temp_index->dev->name: %s", __func__, temp_index->netdev->name);
						}
					} else {
						QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "<%s>  temp_index or dev is NULL ", __func__);
					}
...
void qos_hal_remove_subif_list_item_ingress(struct qos_hal_user_subif_abstract **obj_head,
		uint32_t qosport, uint32_t sub_if)
{
	struct qos_hal_user_subif_abstract *p_prev, *p_cur;
	p_prev = NULL;
	...
	qos_hal_subif_lock_list();
	for (p_cur = *obj_head; p_cur; p_prev = p_cur, p_cur = p_cur->next) {
		if (p_cur->qos_port_idx == qosport && p_cur->dqid == sub_if) {
			pr_debug("Remove subif--> %d %d\n", qosport, sub_if);
			if (!p_prev)
				*obj_head = p_cur->next;
			else
				p_prev->next = p_cur->next;
...
int32_t qos_hal_lookup_subif_list_item_ingress(uint32_t qosport,
			uint32_t sub_if,
			...
			struct qos_hal_user_subif_abstract **obj_head)
{
	...
	for (p = *obj_head; p; p = p->next) {
		if (p != NULL) {
			QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE,"qosport:%d p->qos_port_idx:%d p->dqid:%d sub_if:%d\n",
				qosport, p->qos_port_idx, p->dqid, sub_if);
			if (p->qos_port_idx == qosport && p->dqid == sub_if) {
			...
			struct qos_hal_user_subif_abstract **obj_head)
{
	...
	if (qos_hal_lookup_subif_list_item(qosport, dev, obj, obj_head) == QOS_MGR_SUCCESS) {
		return QOS_MGR_SUCCESS;
		 ...
		 QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "%s() Ifname:%s not found with subif list.\n",
...
int32_t qos_hal_get_subif_list_item_ingress(uint32_t qosport,
			uint32_t sub_if,
			struct net_device *dev,
			struct qos_hal_user_subif_abstract **obj,
			struct qos_hal_user_subif_abstract **obj_head)
{
	...
	if (qos_hal_lookup_subif_list_item_ingress(qosport, sub_if, dev, obj, obj_head) == QOS_MGR_SUCCESS)
		return QOS_MGR_SUCCESS;

	QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE,"No match found for qosport:%d sub_if:%d Allocating new node ...\n", qosport, sub_if);
	p = qos_hal_alloc_subif_item();
	if (!p) {
		pr_err("Obj error\n");
		return QOS_MGR_FAILURE;
	...
	if (dev != NULL) {
		...
		QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE,"Added port index %d p->dqid:%d for Ingress\n", qosport, sub_if);
		p->qos_port_idx = qosport;
		p->dqid = sub_if;
		p->netdev = NULL;
	}
		...
		uint32_t flags)
{
	...
	*res = NULL;
			...
			if (dp_if->port_id != -1) {
				...
				if ((dp_subif->alloc_flag & DP_F_VUNI) && (dp_subif->data_flag & DP_SUBIF_VANI)) {
					QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "<%s> PON interface\n",
							__func__);
					p_dp_subif = qos_mgr_malloc(sizeof(dp_subif_t));
					if (p_dp_subif == NULL) {
						goto err;
					}
					...
					kfree(p_dp_subif);
				} else if (!(dp_subif->alloc_flag & (DP_F_FAST_WLAN|DP_F_FAST_WLAN_EXT)) && (nRet != QOS_MGR_SUCCESS)) {
	...
	QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "<%s> alloc_flag =%x flags=%x\n",
			__func__, dp_subif->alloc_flag, flags);
	...
	if ((flags & QOS_MGR_Q_F_SPCL_CONN) == QOS_MGR_Q_F_SPCL_CONN)
		is_lro = 1;
	...
	if (!is_lro) {
		...
		dq_res.q_res = NULL;
		if (dq_res.num_q) {
			dq_res.q_res = qos_mgr_malloc(sizeof(struct dp_queue_res) * dq_res.num_q);
		...
		if (dp_spl_conn_get(0, DP_SPL_TOE, &conn, 1) != 0) {
			...
			QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "dq_res.q_res->q_id:%d dq_res.q_res->cqm_deq_port:%d\n",
	...
	*res = qos_res = dq_res.q_res;
	...
	/* Move default Lower Prio Queue on CPU High Port by one node for protected high prio queues */
	if ((flags & QOS_MGR_Q_F_INGRESS) == QOS_MGR_Q_F_INGRESS) {
		if (flags & QOS_MGR_Q_F_INGRESS_DEQ_PORT_HIGH) {
			if (dq_res.q_res) {
				...
				*/
				sch_id = (qos_res + (dq_res.num_q - 1))->sch_id[0];
				prio_info.inst = g_Inst;
				prio_info.id.sch_id = sch_id;
				prio_info.type = DP_NODE_SCH;
				if (dp_qos_link_prio_get(&prio_info, 0) == DP_FAILURE) {
	...
	if (qos_res != NULL) {
		if (qos_res->cqm_deq_port > g_No_Of_QOS_Res_Port) {
			QOS_HAL_DEBUG_MSG(QOS_DEBUG_ERR, "qos_res->cqm_deq_port > g_No_Of_QOS_Res_Port\n");
			goto err;
		}
	...
	if (((flags & QOS_MGR_OP_F_ADD) == QOS_MGR_OP_F_ADD) &&
		(qos_hal_port_track[qos_res->cqm_deq_port].no_of_queues ==
				QOS_HAL_MAX_QUEUE_PER_EGRESS_INTERFACE)) {
		QOS_HAL_DEBUG_MSG(QOS_DEBUG_ERR, "<%s> Max No of Qs reached for DP Port %d; Skip queue creation.\n",
	...
	for (i = 0; i < no_of_qos_ports; i++) {
		...
		if ((flags & QOS_MGR_Q_F_INGRESS) == QOS_MGR_Q_F_INGRESS) {
			...
			qos_hal_get_subif_list_item_ingress(ingress_port,
					sub_if,
					NULL,
					&ingress_subif_index,
					&base_subif_index->head);
			qos_port_index = ingress_subif_index->qos_port_idx;
			QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "<%s> INGRESS base_subif_index: %p, base_subif_index->head:%p ingress_subif_index:%p\n",
					__func__, base_subif_index, base_subif_index->head, ingress_subif_index);
		} else {
				...
				if (!logical_dev) {
					...
					if ((qos_res+i)->sch_lvl == 0) {
						QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "sch_lvl=0 and policy=%d\n",
								sched_type);
						if (!IS_ENABLED(CONFIG_SOC_PRX) || (flags & QOS_MGR_Q_F_SP_WFQ))
							sched_type = QOS_HAL_POLICY_WSP; /* Base sched should be always SP */
						connected_sched = create_new_scheduler(
								...
								1);
						if (connected_sched == QOS_HAL_STATUS_ERR) {
							QOS_HAL_DEBUG_MSG(QOS_DEBUG_ERR, "<%s> Failed to add scheduler\n",
									__func__);
							goto err;
						}
...
int qos_hal_ll_init(void)
{
	...
	g_CPU_Count = num_online_cpus();
	QOS_HAL_DEBUG_MSG(QOS_DEBUG_TRACE, "This system has %d processors available.\n", g_CPU_Count);
	...
	for (i = 0; i < port; i++) {
		...
		qos_hal_port_track[i].no_of_queues = 0;
...
QOS_RESOURCE_FREE:
	...
	qos_mgr_lock_init(&g_qos_hal_list_lock);
	return ret;
}

void qos_hal_ll_exit(void)
{
	qos_mgr_lock_destroy(&g_qos_hal_list_lock);
	free_pages_exact(qos_hal_user_sub_interface_info,
		((g_No_Of_QOS_Res_Port + 1) * sizeof(struct qos_hal_user_subif_abstract)));
	free_pages_exact(qos_hal_user_sub_interface_ingress_info,
		((g_No_Of_Ingress_Interface + 1) * sizeof(struct qos_hal_user_subif_abstract)));
}
