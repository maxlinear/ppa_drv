...
#endif
static int proc_read_ingress_control_seq_open(struct inode *, struct file *);
static int proc_read_ingress_control(struct seq_file *, void *);
static int proc_read_codel_seq_open(struct inode *, struct file *);
static int proc_read_codel(struct seq_file *, void *);
...
#endif
static ssize_t proc_write_ingress_control(struct file *file, const char __user *buf, size_t count, loff_t *data);
static ssize_t proc_write_codel(struct file *file, const char __user *buf, size_t count, loff_t *data);
...
static struct file_operations g_proc_file_ingress_control_seq_fops = {
	.owner      = THIS_MODULE,
	.open       = proc_read_ingress_control_seq_open,
	.read       = seq_read,
	.write	    = proc_write_ingress_control,
	.llseek     = seq_lseek,
	.release    = single_release,
};

static int proc_read_ingress_control_seq_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_read_ingress_control, NULL);
}

static int proc_read_ingress_control(struct seq_file *seq, void *v)
{
	if (!capable(CAP_NET_ADMIN)) {
		printk ("Read Permission denied");
		return 0;
...
static ssize_t proc_write_ingress_control(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
	...
	if (strincmp(p, "enable ", 6) == 0) {
		snprintf(buf1, 20, "%s", p);
		gIngressQosEnable = 1;
	} else if (strincmp(p, "disable", 7) == 0) {
		printk("Disable not supported yet.\n");
		dump_fapi_ingress_config();
	} else {
		snprintf(buf1, 20, "%s", p);
		gIngressQosEnable = 1;
		qos_mgr_configure_ingress_queues(buf1);
	}
usage:
		printk("echo <all/eth1 (name of interface of an ingress group)/disable> > /sys/kernel/debug/qos_mgr/ingress_control\n");
	return count;
}

static struct file_operations g_proc_file_codel_seq_fops = {
	.owner      = THIS_MODULE,
	.open       = proc_read_codel_seq_open,
	.read       = seq_read,
	.write	    = proc_write_codel,
	.llseek     = seq_lseek,
	.release    = single_release,
};

static int proc_read_codel_seq_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_read_codel, NULL);
}

static int proc_read_codel(struct seq_file *seq, void *v)
{
	struct dp_qos_codel_cfg cfg = {0};
	...
	if (dp_qos_codel_cfg_get(&cfg, 0) != QOS_MGR_SUCCESS)
		printk("dp_qos_codel_cfg_get failed.\n");
	else
		printk("target_delay_msec:%u interval_time_msec:%u \n",
...
static ssize_t proc_write_codel(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
	...
	len = min(count, (size_t)(sizeof(str) - 1));
	qos_mgr_copy_from_user(str, buf, len);
	str[len] = '\0';
	...
	if (dp_qos_codel_cfg_set(&cfg, 0) != QOS_MGR_SUCCESS)
		printk("dp_qos_codel_cfg_set failed.\n");
	else
		printk("Codel system configuration set to -> target_delay_msec:%u interval_time_msec:%u \n",
				cfg.target_delay_msec,
				cfg.interval_time_msec);
...
usage:
	printk("echo <target_delay_msec> <interval_time_msec> > /sys/kernel/debug/qos_mgr/codel\n");
	return count;
}
...
{
	int len;
	int f_spl_conn, device_type, size;
	char str[64];
	char *p;
	bool oper = 0;
	...
	if (!*p) {
		...
		printk("echo <lroqadd> <queue shaper size> > /sys/kernel/debug/qos_mgr/spl_conn_control\n");
		return count;
	}
	if (strincmp(p, "lroqadd", 6) == 0) {
		device_type = DP_SPL_TOE;
		p += 7 + 1; /*skip enable and one blank*/
		len -= 7 + 1; /*len maybe negative now if no other params*/
		f_spl_conn = 1;
		oper = 1;
	} else if (strincmp(p, "lro", 3) == 0) {
		device_type = DP_SPL_TOE;
	...
	if (f_spl_conn) {
		if (len <= 0) {
			...
			if (device_type == DP_SPL_TOE) {
				qos_hal_spl_conn_mod(DP_SPL_TOE, oper, size);
			}
		}
	}
...
static ssize_t proc_write_qos_mgr_info(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
	...
	if (strincmp(p, "queue ", 5) == 0) {
		...
		qos_hal_dump_qos_mgr_info(Atoi(ptr), "subif");

		goto end;
	}

usage:
	printk("echo <queue/shaper/port/sched/subif> <index> > /sys/kernel/debug/qos_mgr/qos_mgr_info\n");
	printk("To print all use:\n echo <queue/shaper/port/sched/subif> -1 > /sys/kernel/debug/qos_mgr/qos_mgr_info\n");
...
int qos_hal_proc_create(void)
{
	...
	file = debugfs_create_file("ingress_control", 0600,
			g_qos_mgr_proc_dir, NULL, &g_proc_file_ingress_control_seq_fops);
	if (!file)
		goto err;

	file = debugfs_create_file("codel", 0600,
			g_qos_mgr_proc_dir, NULL, &g_proc_file_codel_seq_fops);
	if (!file)
		goto err;
