...
#define CPU_LOW 0
#define CPU_HIGH 1
#define INGRESS_OPTION_0 0 /* Each Group: 1 Low Prio Q, 1 High Prio Q */
#define INGRESS_OPTION_1 1 /* Each Group: 1 Low Prio Q, Shared High Prio Q (DPM Queue) */
#define LGM_CPU_PORT_COUNT 4
#define LGM_INGRESS_MAX_PRIO 4
#define MAX_INGRESS_QUEUE_PER_GROUP_PER_PORT 8
#define QOS_MGR_Q_F_INGRESS_DEQ_PORT_LOW 0
#define QOS_MGR_Q_F_INGRESS_DEQ_PORT_HIGH 1
#define MAX_INGRESS_PROTECTED_QUEUES_PER_PORT 7
...
#define QOS_MGR_MAX_NUM_RULES_PER_GROUP	10
...
uint32_t inggrp2flags(QOS_MGR_INGGRP inggrp)
{
	...
	case QOS_MGR_INGGRP4: return QOS_MGR_Q_F_INGGRP5;
	case QOS_MGR_INGGRP5: return QOS_MGR_Q_F_INGGRP6;
	case QOS_MGR_INGGRP6: return QOS_MGR_Q_F_INGGRP7;
...
struct qos_mgr_user_queue_info {
	/** Actual Queue index */
 ...
 */
typedef struct qos_inggrp_list_item {
	...
	uint32_t                            	queue_count_high;
	uint32_t                            	queue_count_low;
	bool                                	fapi_reg_status;
	bool                                	dp_reg_status;
	int32_t				    	pp_grp_id;
	uint32_t			    	gpids[QOS_MGR_MAX_IF_PER_INGGRP];
	char                                	ifnames[MAX_IF_PER_CPU_INGRESS_GROUP][QOS_MGR_IFNAME_SIZE];/*!< Ingress Interface name corresponding to a Ingress QoS group*/
	uint8_t				    	if_count;
	int				    	num_ing_rules;
	QOS_MGR_CMD_ADD_CLASSIFIER_INFO		ing_class_rule_cfg[QOS_MGR_MAX_NUM_RULES_PER_GROUP];
	struct qos_mgr_user_queue_info      	user_queue_high[LGM_CPU_PORT_COUNT][MAX_INGRESS_QUEUE_PER_GROUP_PER_PORT];
	struct qos_mgr_user_queue_info      	user_queue_low[LGM_CPU_PORT_COUNT][MAX_INGRESS_QUEUE_PER_GROUP_PER_PORT];
...
typedef enum {
	...
	QOS_MGR_GENERIC_HAL_QOS_ADD_CLASSIFIER_CFG,
...
void qos_hal_global_wmm_status(struct seq_file *);
int32_t qos_hal_spl_conn_mod(uint8_t conn_type, bool oper, uint32_t queue_len);

void qos_mgr_configure_ingress_queues(const char *ifname);
