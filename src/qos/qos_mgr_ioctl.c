...
long qos_mgr_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if ( ((qos_mgr_ioc_dir(cmd) & qos_mgr_ioc_read()) && !qos_mgr_ioc_access_ok(qos_mgr_ioc_verify_write(), arg, qos_mgr_ioc_size(cmd)))
			|| ((qos_mgr_ioc_dir(cmd) & qos_mgr_ioc_write()) && !qos_mgr_ioc_access_ok(qos_mgr_ioc_verify_read(), arg, qos_mgr_ioc_size(cmd))) ) {
	...
	switch ( cmd ) {
		...
		case QOS_MGR_CMD_GET_QOS_MIB: {
			if (!capable(CAP_NET_ADMIN))
				goto EXIT_EFAULT;
			res = qos_mgr_ioctl_get_qos_mib(cmd, arg, cmd_info);
			break;
		}

		case QOS_MGR_CMD_ADD_CLASSIFIER: {
			if (!capable(CAP_NET_ADMIN))
				goto EXIT_EFAULT;
			res = qos_mgr_ioctl_set_classifier(cmd, arg, cmd_info);
			break;
		}
		default:
