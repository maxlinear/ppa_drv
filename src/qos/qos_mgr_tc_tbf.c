		...
		int32_t flags)
{
	...
	rate_info.shaper.cir = div_u64(tbf->replace_params.rate.rate_bytes_ps,
				       1000);
	rate_info.shaper.pir = div_u64(tbf->replace_params.peak.rate_bytes_ps,
				       1000);
			...
			struct tc_tbf_qopt_offload *sch_tbf_off)
{
	...
	case TC_TBF_REPLACE:
		pr_debug("%s: parent %#x handle %#x "
				" rate %llu peak %llu buffer: %lld mtu: %lld\n",
				__func__,
				sch_tbf->parent,
				sch_tbf->handle,
				sch_tbf->replace_params.rate.rate_bytes_ps,
				sch_tbf->replace_params.peak.rate_bytes_ps,
				sch_tbf->replace_params.buffer,
				sch_tbf->replace_params.mtu);
