...
#define NETIF_GOING_DOWN	0x100000000
...
#define NETIF_RXCSUM			((uint64_t)1 << 32)
...
struct netif_info {
	...
	/**
	 *	@netdev_ops:	Saves original net_device_ops pointer
	 *			only for PPA managed netif
	 *	@netdev_ops_new:Saves original net_device_ops callbacks
	 *			for DPM managed netif. Alternatively,
	 *			if PPA managed it contains original
	 *			with the modified callbacks.
	 *	@delta_features:Saves modified device features
	 */
	struct net_device_ops	*netdev_ops_new;
...
int32_t ppa_flow_steer_update(struct net_device *dev, const struct sk_buff *skb, uint16_t dest_q, u32 flow_id);
#endif
/* @} */
