 ...
 */
    ...
    (IS_ENABLED(CONFIG_X86_INTEL_LGM) || IS_ENABLED(CONFIG_SOC_LGM))
...
#endif
uint32_t g_non_accel_session_timeout = DEFAULT_NON_ACCEL_SESSION_TIMEOUT_IN_SEC;
...
static uint32_t pktprs_is_pkt_mc(struct pktprs_hdr *h)
{
	...
	if (PKTPRS_IS_IPV4(h, lvl)) {
		if (ipv4_is_multicast(pktprs_ipv4_hdr(h, lvl)->daddr))
			return 1;
		...
		if (ipv6_addr_is_multicast(&pktprs_ipv6_hdr(h, lvl)->daddr))
			return 1;
	}
#endif
	return 0;
...
static uint16_t pktprs_l4_proto(struct pktprs_hdr *h)
{
	...
	if (PKTPRS_IS_UDP(h, lvl) && !PKTPRS_IS_L2TP_OUDP(h, lvl))
...
static int32_t pktprs_is_ip_frag_not_first(struct pktprs_hdr *h)
{
	...
	/* internal IP frag */
	if (PKTPRS_IS_MULTI_IP(h)) {
		if (pktprs_is_ip_frag(h, PKTPRS_HDR_LEVEL1) &&
			!pktprs_first_frag(h, PKTPRS_HDR_LEVEL1))
			return 1;
	}
...
static int32_t pktprs_is_loopback_pkt(struct pktprs_hdr *inh,
				      struct pktprs_hdr *outh)
{
	uint8_t lvl;
	...
	if (!inh->ifindex && outh && !outh->ifindex)
		return 1;

	lvl = get_correct_hdr_lvl(inh);
	if (PKTPRS_IS_IPV6(inh, lvl)) {
		if (ppa_memcmp(pktprs_ipv6_hdr(inh, lvl)->saddr.in6_u.u6_addr32,
			       pktprs_ipv6_hdr(inh, lvl)->daddr.in6_u.u6_addr32,
			       sizeof(PPA_IPADDR)) == 0) {
			return 1;
		}
	} else if (PKTPRS_IS_IPV4(inh, lvl)) {
		if (pktprs_ipv4_hdr(inh, lvl)->saddr ==
		    pktprs_ipv4_hdr(inh, lvl)->daddr) {
			return 1;
		}
	}
	return 0;
...
static uint16_t pktprs_src_port(struct pktprs_hdr *h)
{
	...
	if (PKTPRS_IS_TCP(h, lvl))
		...
		port = (pktprs_esp_hdr(h, lvl)->spi >> 16) & 0xFFFF;
...
static uint16_t pktprs_dst_port(struct pktprs_hdr *h)
{
	...
	if (PKTPRS_IS_TCP(h, lvl))
		...
		port = (pktprs_esp_hdr(h, lvl)->spi) & 0xFFFF;
...
static uint32_t pktprs_vxlan_info(struct pktprs_hdr *h,
				struct uc_session_node *p_item)
{
	if (PKTPRS_IS_VXLAN(h, PKTPRS_HDR_LEVEL0)) {
				...
				uint32_t flags)
{
	...
	/* Session exists but with temporary error, so allow to learn it again! */
	if (p_item->flags & SESSION_NOT_ACCEL_FOR_MGM) {
		p_item->num_adds++;
		if (p_item->num_adds > (10 * g_ppa_min_hits)) {
			p_item->flags &= ~SESSION_NOT_ACCEL_FOR_MGM;
			p_item->num_adds = g_ppa_min_hits - 1;
		}
	}
	...
	if (!(flags & PPA_F_BRIDGED_SESSION)) {
		...
		// local_in/out non tcp session not accelerated
			if (!(proto == PPA_IPPROTO_UDP) &&
				(flags & (PPA_F_SESSION_LOCAL_IN|PPA_F_SESSION_LOCAL_OUT))) {
				p_item->flags |= SESSION_NOT_ACCELABLE;
				SET_DBG_FLAG(p_item, SESSION_DBG_ALG);
				goto __PPA_FILTERED;
			}
				...
				uint32_t flags)
{
	...
	*/
	...
	if (ppa_get_pkt_priority(ppa_buf) == 0) {
		...
		mask |= SESSION_DS_QOS_SET;
#endif
#ifdef HAVE_QOS_EXTMARK
#ifdef CONFIG_NETWORK_EXTMARK
		pkt_mark |= CPU_CLASSIFICATION_MASK;
		mask |= CPU_CLASSIFICATION_MASK;
#endif
		ppa_set_skb_extmark(ppa_buf, pkt_mark, mask);
#else /* HAVE_QOS_EXTMARK */
		SET_DATA_FROM_MARK_OPT(ppa_buf->mark, mask, 0, pkt_mark);
...
__UPDATE_PROCESSED:
	...
	if ((ret = ppa_hsel_add_routing_session(p_item, ppa_buf,
		NULL, hsel_flags)) != PPA_SUCCESS) {
...
#ifdef HAVE_QOS_EXTMARK
		pkt_mark = p_item->pkt.extmark;
#else
		pkt_mark = p_item->pkt.mark;
#endif /* HAVE_QOS_EXTMARK */

		/* DS_QOS valid only for WAN sessions */
		if (pkt_mark & SESSION_DS_QOS_SET)
			...
			uint32_t flags, struct uc_session_node **pp_item)
{
	...
	** we need to mark this session item as SESSION_FLAG2_CPU_OUT as that it can be checked later for taking action*/
	if (ppa_is_pkt_host_output(ppa_buf)
		&& ((ppa_get_pkt_ip_proto(ppa_buf) == PPA_IPPROTO_TCP)
	...
	if (!p_item) {
		ret = PPA_ENOMEM;
		ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT, "failed in memory allocation\n");
		ppa_netif_put(rx_ifinfo);
		goto __ADD_SESSION_DONE;
	}
	...
	p_item->last_hit_time_jif = jiffies;
#endif
	p_item->flag2 |= flg2;
...
static void ppa_free_mc_session_node(struct mc_session_node *p_item)
{
	...
	p_item->grp.sess_info->ref_count--;
	if (!p_item->grp.sess_info->ref_count)
		ppa_free(p_item->grp.sess_info);
	p_item->grp.sess_info = NULL;
...
{
	uint32_t idx;
	idx = SESSION_LIST_MC_HASH_VALUE(p_item->grp.sess_info->ip_mc_group.ip.ip);
...
void ppa_check_hit_stat_clear_mib(int32_t flag)
{
	...
	for (i = 0; i < NUM_ENTITY(g_session_list_hash_table); i++) {
		...
		ppa_hlist_for_each_entry(p_item, &g_session_list_hash_table[i], hlist) {
			...
			if ((p_item->flags & (SESSION_ADDED_IN_HW|SESSION_ADDED_IN_SW)) == SESSION_ADDED_IN_HW) {
				route.packets = 0;
				route.bytes = 0;
				route.f_hit = 0;
				...
				if (route.f_hit) {
...
#if defined(PPA_IF_MIB) && PPA_IF_MIB && IS_ENABLED(CONFIG_SOC_GRX500)
					if (p_item->rx_if)
					...
					/* Conntrack manager will not remove any inactive session from PPA until
					 * the conntrack entry crosses the timeout set for each protocol (sysctl -a | grep timeout).
					 * In worst case scenario, and inactive session will not be removed for 7440 seconds.
					 * But with the below, no inactive session will last longer than DEFAULT_SESSION_TIMEOUT_IN_SEC
					 */
					if (((ppa_get_time_in_sec() - p_item->last_hit_time) > DEFAULT_SESSION_TIMEOUT_IN_SEC)) {
						__ppa_session_delete_item(p_item);
					}
				}
				...
				if (tmp > 0) {
					 ...
					 * But with the below, no inactive session will last longer than DEFAULT_SESSION_TIMEOUT_IN_SEC
					 */
					if ((ppa_get_time_in_sec() - p_item->last_hit_time) >
					    DEFAULT_SESSION_TIMEOUT_IN_SEC)
						__ppa_session_delete_item(p_item);
				}
			} else {
				/* non-accelerated session timed out */
				if ((ppa_get_time_in_sec() - p_item->last_hit_time) >
	...
	for (i = 0; i < NUM_ENTITY(g_mc_group_list_hash_table); i++) {
		...
		ppa_hlist_for_each_entry(p_mc_item, &g_mc_group_list_hash_table[i], mc_hlist) {
			...
			if (p_mc_item->flags & SESSION_ADDED_IN_HW) {
				bool shared_dup = false;

				/*
				 * NOTE: Multiple p_item may share single sess_info tuple.
				 * For the first only,
				 *   get counter from the respective hal and
				 *   save it as part of shared sess_info.
				 * Otherwise, get counter from shared sess_info.
				 */
				mc.f_hit = 0; mc.bytes = 0;
				if (p_mc_item->acc_bytes == p_mc_item->grp.sess_info->acc_bytes)
					ppa_hsel_get_mc_entry_bytes(&mc, flag, p_mc_item->caps_list[0].hal_id);
				else {
					mc.f_hit = 1;
					mc.bytes = p_mc_item->grp.sess_info->acc_bytes - p_mc_item->acc_bytes;
					shared_dup = true;
				}
				if (mc.f_hit) {
					...
					if (!shared_dup)
						p_mc_item->grp.sess_info->acc_bytes = p_mc_item->acc_bytes;
#if defined(PPA_IF_MIB) && PPA_IF_MIB
					if (p_mc_item->grp.src_netif && !shared_dup)
	...
	queue_delayed_work(g_timer_workq, &check_hit_stat_dwork,
				msecs_to_jiffies(g_hit_polling_time * MSEC_PER_SEC));
}
...
void dump_list_item(struct uc_session_node *p_item, char *comment)
{
	...
	printk("  last_hit_time (sec)	= %d\n", p_item->last_hit_time);
#if IS_ENABLED(CONFIG_X86_INTEL_LGM) || IS_ENABLED(CONFIG_SOC_LGM)
	printk("  last_hit_time	(jif) = %d\n",   p_item->last_hit_time_jif);
#endif
	printk("  num_adds		 = %d\n",   p_item->num_adds);
...
int32_t __ppa_lookup_mc_group(IP_ADDR_C *ip_mc_group, IP_ADDR_C *src_ip, struct mc_session_node **pp_item)
{
	...
	ppa_hlist_for_each_entry(p_mc_item,&g_mc_group_list_hash_table[idx],mc_hlist) {
		if (ip_equal(&p_mc_item->grp.sess_info->ip_mc_group, ip_mc_group)) { /*mc group ip match*/
			if (ip_equal(&p_mc_item->grp.sess_info->source_ip, src_ip)) {/*src ip match*/
				*pp_item = p_mc_item;
				return PPA_SESSION_EXISTS;
			} else {
				if (is_ip_zero(&p_mc_item->grp.sess_info->source_ip) || is_ip_zero(src_ip)) {
					*pp_item = NULL;
...
int32_t __ppa_lookup_mc_gid(IP_ADDR_C *ip_mc_group, uint16_t mc_gid, struct mc_session_node **pp_item)
{
	uint32_t idx;
	struct mc_session_node *p_mc_item = NULL;
	...
	idx = SESSION_LIST_MC_HASH_VALUE(ip_mc_group->ip.ip);
	ppa_hlist_for_each_entry(p_mc_item,&g_mc_group_list_hash_table[idx], mc_hlist) {
		if (p_mc_item->grp.group_id == mc_gid) {
			*pp_item = p_mc_item;
			return PPA_SESSION_EXISTS;
		} else {
			if ((p_mc_item->grp.group_id == 0) || (mc_gid == 0)) {
				*pp_item = NULL;
				return PPA_MC_SESSION_VIOLATION;
			}
		}
...
void ppa_delete_mc_group(PPA_MC_GROUP *ppa_mc_entry)
{
	...
	ppa_hlist_for_each_entry_safe(p_mc_item,node_next,&g_mc_group_list_hash_table[idx],mc_hlist) {
		if (p_mc_item->grp.group_id == ppa_mc_entry->group_id) {
			ppa_remove_mc_group_item(p_mc_item);
			p_mc_item->flags &= ~SESSION_DROP;
			ppa_free_mc_session_node(p_mc_item);
			break;
		}
	}

	/* Allow to re-learn the existing session into PAE/MPE */
	if (IS_ENABLED(CONFIG_SOC_GRX500)) {
		int32_t ret;
		struct mc_session_node *p_mc_item = NULL;

		ret = __ppa_lookup_mc_group(&ppa_mc_entry->ip_mc_group, &ppa_mc_entry->source_ip, &p_mc_item);
		if ((ret == PPA_SESSION_EXISTS) &&
		    (p_mc_item->grp.sess_info->ref_count == 1) &&
		    p_mc_item->num_caps && (p_mc_item->caps_list[0].hal_id == MPE_HAL)) {
			ppa_hsel_del_wan_mc_group(p_mc_item);
			p_mc_item->num_adds = 0;
			p_mc_item->num_caps = 0;
			ret = ppa_hsel_add_wan_mc_group(p_mc_item, NULL);
			if (ret != PPA_SUCCESS)
...
static int32_t ppa_mc_check_src_itf(char *itf, struct mc_session_node *p_item)
{
	...
	if (itf != NULL) {
		if (ppa_mc_itf_get(itf, &p_netif_info) != PPA_SUCCESS)
			return PPA_FAILURE;
		/* FIXME: what happens if srcif is under another bridge? */
...
static int32_t ppa_mc_check_dst_itf(PPA_MC_GROUP *p_mc_group, struct mc_session_node *p_item)
{
	...
	for (i = 0; i < PPA_MAX_MC_IFS_NUM; i++) {
		...
		if (ppa_mc_itf_get(p_mc_group->array_mem_ifs[i].ifname, &p_netif_info) != PPA_SUCCESS) {
			ppa_debug(DBG_ENABLE_MASK_ERR, "mc dst itf get fail for %s!\n",
				p_mc_group->array_mem_ifs[i].ifname);
			return PPA_FAILURE;
		}

		if (!(p_netif_info->flags & NETIF_PHYS_PORT_GOT)) {
			ppa_debug(DBG_ENABLE_MASK_ERR, "phys port missing for %s!\n",
				p_netif_info->name);
			ppa_netif_put(p_netif_info);
			return PPA_FAILURE;
		}
...
#if IS_ENABLED(CONFIG_X86_INTEL_LGM) || IS_ENABLED(CONFIG_SOC_LGM)
			/* Use base ifindex of 1st listener of the grp to identify the duplicated pkts in RTP port learning */
			p_item->rtp_info.tx_ifindex = ppa_get_netif_ifindex(p_netif_info->phys_netif_name);
#endif /*IS_ENABLED(CONFIG_X86_INTEL_LGM) || IS_ENABLED(CONFIG_SOC_LGM)*/
		} else {
...
/* In case of LGM we dont extract the RX netdevice information at this point*/
	...
	if (isIgEoGre) {
		...
		if (tmp_src_netif && netif_is_bridge_port(tmp_src_netif)) {
	...
	if (ppa_mc_check_dst_itf(p_mc_group, p_item) != PPA_SUCCESS) {
		ppa_debug(DBG_ENABLE_MASK_ERR, "mc dst itf check fail!\n");
		return PPA_FAILURE;
	}

	/*multicast  address*/
	ppa_memcpy(&p_item->grp.sess_info->ip_mc_group, &p_mc_group->ip_mc_group, sizeof(p_mc_group->ip_mc_group));
	/*source ip address*/
	ppa_memcpy(&p_item->grp.sess_info->source_ip, &p_mc_group->source_ip, sizeof(p_mc_group->source_ip));
...
int32_t ppa_add_mc_group(PPA_MC_GROUP *p_mc_group, struct mc_session_node **pp_item, uint32_t flags)
{
	...
	/* Update session info reference */
	ret = __ppa_lookup_mc_group(&p_mc_group->ip_mc_group, &p_mc_group->source_ip, &p_mc_item);
	if (ret == PPA_SESSION_EXISTS)
		sess_info = p_mc_item->grp.sess_info;

	if (!sess_info) {
		sess_info = ppa_malloc(sizeof(MC_SESS_INFO));
		if (!sess_info) {
			ppa_mem_cache_free(p_item, g_mc_group_item_cache);
			return PPA_ENOMEM;
		}
		ppa_memset(sess_info, 0, sizeof(MC_SESS_INFO));
	}
	sess_info->ref_count++;
	p_item->grp.sess_info = sess_info;

	/* Allow to re-learn the existing session into MPE only */
	if (IS_ENABLED(CONFIG_SOC_GRX500) && (sess_info->ref_count == 2) &&
	    p_mc_item && p_mc_item->num_caps &&
	    (p_mc_item->caps_list[0].hal_id != MPE_HAL)) {
		ppa_hsel_del_wan_mc_group(p_mc_item);
		p_mc_item->num_adds = 0;
		p_mc_item->num_caps = 0;
		ret = ppa_hsel_add_wan_mc_group(p_mc_item, NULL);
...
int32_t ppa_mc_group_iterate_next(uint32_t *ppos, struct mc_session_node **pp_item)
{
	...
	if ((*pp_item)->mc_hlist.next != NULL) {
		...
		*pp_item = ppa_hlist_entry((*pp_item)->mc_hlist.next,
		...
		for (idx = SESSION_LIST_MC_HASH_VALUE((*pp_item)->grp.sess_info->ip_mc_group.ip.ip) + 1;
...
int32_t ppa_ipsec_add_entry(uint32_t tunnel_index)
{
	...
	if (ppa_hw_accelerate(NULL, p_item)) {
		if ((ret = ppa_hsel_add_routing_session(p_item, NULL,
			NULL, 0)) != PPA_SUCCESS) {
...
*/
int32_t __ppa_mc_group_update(PPA_MC_GROUP *p_mc_entry,
			struct mc_session_node *p_item, uint32_t flags)
{
	...
	/* Update session info reference */
	p_mc_item->grp.sess_info = p_item->grp.sess_info;
	...
	if (IS_ENABLED(CONFIG_SOC_GRX500) &&
	    (p_item->grp.sess_info->ref_count == 1)) {
		...
		if (ppa_hsel_add_wan_mc_group(p_mc_item, &p_mc_entry->cop) != PPA_SUCCESS)
			ppa_debug(DBG_ENABLE_MASK_ERR, "can not update %s %d\n", __FUNCTION__, __LINE__);
	} else {
		/*Copy necesary information from the original p_item */
		p_mc_item->mc_entry = p_item->mc_entry;
		p_mc_item->grp.priority = p_item->grp.priority;
#ifdef HAVE_QOS_EXTMARK
		p_mc_item->grp.qos_mark.extmark = p_item->grp.qos_mark.extmark;
#else
		p_mc_item->grp.qos_mark.mark = p_item->grp.qos_mark.mark;
#endif
...
int32_t ppa_mc_entry_rtp_set(PPA_MC_GROUP *ppa_mc_entry)
{
	...
	ppa_mc_get_htable_lock();
	ppa_hlist_for_each_entry_safe(p_mc_item, node_next, &g_mc_group_list_hash_table[idx], mc_hlist) {
		if (ip_equal(&p_mc_item->grp.sess_info->ip_mc_group, &ppa_mc_entry->ip_mc_group)) {/*mc group ip match*/
			if (is_ip_zero(&ppa_mc_entry->source_ip) ||
			    ip_equal(&p_mc_item->grp.sess_info->source_ip, &ppa_mc_entry->source_ip)) {
				entry_found = 1;
				...
				if (ip_equal(&p_mc_item->grp.sess_info->source_ip, &ppa_mc_entry->source_ip)) {
					break;
				}
			}
		}
...
int32_t ppa_bridging_add_mac(uint8_t *mac, uint16_t fid, PPA_NETIF *netif, struct br_mac_node **pp_item, uint32_t flags)
{
	...
	dump_bridging_list_item(p_item, "ppa_bridging_add_mac(after init)");
	...
	dump_bridging_list_item(p_item, "ppa_bridging_add_mac(after setting)");
...
void dump_bridging_list_item(struct br_mac_node *p_item, char *comment)
{
	...
	pr_info("  next            = %p\n", &p_item->br_hlist);
	pr_info("  mac[6]          = %pM\n", p_item->mac);
	pr_info("  netif           = %s\n", (p_item->netif ? p_item->netif->name : "NULL"));
	pr_info("  timeout         = %u\n", p_item->timeout);
	pr_info("  last_hit_time   = %d\n", p_item->last_hit_time);
	pr_info("  flags           = %08X\n", p_item->flags);
	pr_info("  bridging_entry  = %08X\n", p_item->bridging_entry);
	pr_info("  ref_count       = %d\n", p_item->ref_count);
#endif
}
...
extern bool is_pp_sess_valid(int32_t sess_id);
...
int32_t ppa_session_entry_check(struct uc_session_node* p_item, PPA_BUF *skb)
{
	...
	if ((p_item->flags & SESSION_IS_TCP) && p_item->session) {
		...
		/* check whether the session has more than g_ppa_min_hits */
		if (ppa_session_entry_pass(p_item->num_adds, g_ppa_min_hits)) {
			ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,
				    "[%s:%d] Can not accelerate as p_item->num_adds %d < g_ppa_min_hits %d\n",
				    __FUNCTION__, __LINE__, p_item->num_adds, g_ppa_min_hits);
			return PPA_FAILURE;
		}
	}
...
#ifdef HAVE_QOS_EXTMARK
	pkt_mark = ppa_get_skb_extmark(ppa_buf);
	GET_DATA_FROM_MARK_OPT(pkt_mark, ACCELSEL_MASK, ACCELSEL_START_BIT_POS, accel_st);
#else
	pkt_mark = ppa_get_skb_mark(ppa_buf);
	GET_DATA_FROM_MARK_OPT(pkt_mark, MARK_ACCELSEL_MASK, MARK_ACCELSEL_START_BIT_POS, accel_st);
#endif /* HAVE_QOS_EXTMARK */
	...
	uint32_t flags, uint16_t last_ethtype)
{
	...
	if (pp_accl_mode_get(&pp_accl_mode))
		ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"failed to get pp acceleration mode %s %d\n", __FUNCTION__, __LINE__);

	/* ignore un supported protocols */
	if (!(l4_proto == PPA_IPPROTO_TCP || l4_proto == PPA_IPPROTO_UDP || l4_proto == PPA_IPPROTO_ESP)) {
		if (pp_accl_mode == PP_ACCL_MODE_UNKNOWN_L3) {
			/* Do no accelerate ARP in unknown L3 mode */
			if (last_ethtype == ETH_P_ARP) {
				ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"can not accelerate %s %d\n", __FUNCTION__, __LINE__);
				ppa_filter.ppa_is_pkt_protocol_invalid++;
				ret = PPA_SESSION_FILTED;
			}
		} else if (pp_accl_mode == PP_ACCL_MODE_NORMAL) {
			/* MOCA CTP acceleration support */
...
#define MAX_PKT_DELAY	1	/* Max delay (in seconds)
				   between 2 consecutive RTP packets */
#define MIN_PKT_HIT	5	/* Min number packets to be inspected
				   before accelerating the session */
#define DELAY_AVG(p_item) \
	(p_item->rtp_info.delay_avg + \
	 (ppa_get_time_in_sec() - p_item->rtp_info.last_hit_time)) / 2;

/* To distinguish between RTP and RTCP packets, read MIN_PKT_HIT number of
 * pacekts. Check the average interval between them. If it is less than
 * 1 sec then it is RTP packet, accelerate it.
 */
static inline int32_t
	mc_check_rtp_info(struct mc_session_node *p_item,
			  struct lgm_mc_args *mc_args)
{
	int32_t ret = PPA_FAILURE;
	uint16_t dport;

	/* To prevent duplicated pkts incremnting the hit counter */
	if (p_item->rtp_info.tx_ifindex != mc_args->desc->tx->ifindex)
		return ret;

	dport = pktprs_dst_port(mc_args->desc->rx);
	if (dport != p_item->rtp_info.dport) {
		p_item->rtp_info.hit_count = 0;
		p_item->rtp_info.delay_avg = 0;
		p_item->rtp_info.dport = dport;
	} else {
		p_item->rtp_info.hit_count++;
		p_item->rtp_info.delay_avg = DELAY_AVG(p_item);
		if (p_item->rtp_info.hit_count > MIN_PKT_HIT &&
		    p_item->rtp_info.delay_avg < MAX_PKT_DELAY)
			ret = PPA_SUCCESS;
		else if (p_item->rtp_info.hit_count > MIN_PKT_HIT)
			p_item->rtp_info.hit_count = 0;
	}
	p_item->rtp_info.last_hit_time = ppa_get_time_in_sec();
...
int32_t ppa_mc_session_entry_check(struct mc_session_node *p_item, struct lgm_mc_args *mc_args)
{
	...
	ppa_memset(&entry, 0, sizeof(FILTER_INFO));
	entry.ip_proto = pktprs_l4_proto(mc_args->desc->rx);
	entry.src_port = pktprs_src_port(mc_args->desc->rx);
	entry.dst_port = pktprs_dst_port(mc_args->desc->rx);

	filter_node = get_matched_entry(PPV4_HAL, &entry);
	if (filter_node) {
		ppa_debug(DBG_ENABLE_MASK_DUMP_MC_GROUP, "mc session proto:%d sport:%d dport%d filtered!\n",
			entry.ip_proto, entry.src_port, entry.dst_port);
		return PPA_FAILURE;
	}
	/* Learn RTP port only though the group session packets */
	if (mc_args->desc->rx->ifindex != 0)
		ret = mc_check_rtp_info(p_item, mc_args);
...
int32_t ppa_mc_add_pp_session(struct mc_session_node *p_mc_item, struct pktprs_desc *desc, struct pp_desc *ppdesc)
{
	...
	if (!ppa_mc_session_entry_check(p_mc_item, &mc_args)) {
		...
		if (desc->rx->ifindex) {
			p_mc_item->grp.priority = ppa_get_pkt_priority(desc->skb);
#ifdef HAVE_QOS_EXTMARK
			p_mc_item->grp.qos_mark.extmark = ppa_get_skb_extmark(desc->skb);
#else
			p_mc_item->grp.qos_mark.mark = ppa_get_skb_mark(desc->skb);
#endif /* HAVE_QOS_EXTMARK */
		}
...
/**
 * @brief Check whether the session's is xlat where ipv4 was translated
 * to ipv6 or vice versa without any other IP tunnels
 * @param pktprs_desc
 * @return bool true if it is, false otherwise
 */
static inline bool ppa_is_xlat_sess(struct pktprs_desc *desc)
{
	return (((PKTPRS_IS_IPV4(desc->rx, PKTPRS_HDR_LEVEL0) && PKTPRS_IS_IPV6(desc->tx,
			PKTPRS_HDR_LEVEL0)) ||
			(PKTPRS_IS_IPV4(desc->tx, PKTPRS_HDR_LEVEL0) && PKTPRS_IS_IPV6(desc->rx,
			PKTPRS_HDR_LEVEL0))) &&
...
int32_t ppa_add_pp_session(struct pktprs_desc *desc, bool drop)
{
	...
	uint16_t last_ethtype = 0, last_ethtype_tx;
	...
	if (pktprs_is_loopback_pkt(desc->rx, desc->tx))
		return PPA_FAILURE;
	...
	if (ppdesc) {
		if (!ppdesc->ud.is_exc_sess && is_pp_sess_valid(ppdesc->ud.sess_id) &&
	...
	ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT, "%s %d ct=%px flags=%d\n",
		  __func__, __LINE__, ct, flags);
	...
	last_ethtype_tx = pktprs_last_ethtype(desc->tx);

	ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,
		  "%s %d l3_proto=%x l4_proto=%x srcport=%d dstport=%d txif=%s "
		  "RX last_ethtype=%x TX last_ethtype=%x\n", __func__, __LINE__,
		  l3_proto, l4_proto, srcport, dstport, !txif ? NULL: txif->name,
		  last_ethtype, last_ethtype_tx);

	if (last_ethtype_tx == ETH_P_MACSEC) {
		ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT, "can not accelerate %s %d\n", __func__, __LINE__);
		ppa_filter.ppa_is_pkt_protocol_invalid++;
		return PPA_FAILURE;
	}
	...
	/* session is not found; we need to add a new session */
		...
		ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT," timeout		= %u\n",   p_item->timeout);
		ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT," last_hit_time (sec)	= %d\n",   p_item->last_hit_time);
#if IS_ENABLED(CONFIG_X86_INTEL_LGM) || IS_ENABLED(CONFIG_SOC_LGM)
		ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT," last_hit_time (jif)	= %d\n",   p_item->last_hit_time_jif);
#endif
	...
	/* session exists update the required parameters */
		...
		if ((p_item->flags & (SESSION_ADDED_IN_HW|SESSION_ADDED_IN_SW)) == SESSION_ADDED_IN_HW) {
			...
			if (is_frag && !(p_item->flag2 & SESSION_FLAG2_FRAG)) {
				...
				ppa_hsel_del_routing_session(p_item);
				goto update_session;
			} else if ((p_item->flags & SESSION_IS_TCP) && p_item->session &&
				!ppa_is_tcp_established(p_item->session)) {
				__ppa_session_put(p_item);
				ppa_session_bucket_unlock(idx);
				ppa_session_delete(p_item->session, 0);
				goto __SKIP_SESSION_ACTION;
			}
		} else if (p_item->flags & SESSION_ADDED_IN_SW) {
...
update_session:
				...
				ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"%s %d update session\n", __FUNCTION__, __LINE__);
				if (!pktprs_vxlan_info(desc->rx, p_item))
					pktprs_vxlan_info(desc->tx, p_item);
				...
				if (p_item->rx_if && rx_ifinfo) {
					if (rx_ifinfo->flags & NETIF_RXCSUM)
						p_item->flag2 |= SESSION_FLAG2_RXCSUM_VALID;
					...
					/* find the receive logical interface using the pktprs */
					t_ifinfo = NULL;
					rxif = ppa_logical_netif_from_pktprs(desc->rx, PKT_RX, p_item->rx_if, &t_ifinfo);
					if (rxif != p_item->rx_if && !(p_item->flags & SESSION_TUNNEL_ESP)) {
				...
				if (p_item->tx_if) {
					...
					/* find the transmit logical interface using the pktprs */
					t_ifinfo = NULL;
					txif = ppa_logical_netif_from_pktprs(desc->tx, PKT_TX, p_item->tx_if, &t_ifinfo);
					if (txif != p_item->tx_if && !(p_item->flags & SESSION_TUNNEL_ESP)) {
...
*/
				...
				if (tx_ifinfo && (tx_ifinfo->flags & NETIF_WAN_IF) &&
				    (!(p_item->flag2 & SESSION_FLAG2_CPU_OUT)))
					p_item->flag2 |= SESSION_FLAG2_ACK_SUPP_ALLOWED;
#endif
				if (((tx_ifinfo && !(tx_ifinfo->flags & NETIF_PHYS_PORT_GOT)) ||
					(rx_ifinfo && !(rx_ifinfo->flags & NETIF_PHYS_PORT_GOT)))) {
					p_item->flags |= SESSION_NOT_VALID_PHY_PORT;
				...
				if (tx_ifinfo) {
					p_item->dest_ifid = tx_ifinfo->phys_port;
					if (tx_ifinfo->flags & NETIF_DIRECTPATH)
						p_item->dest_subifid = tx_ifinfo->subif_id;
					if ((tx_ifinfo->flags & NETIF_PHY_ATM)) {
						fake_skb.priority = ppa_get_pkt_priority(ppa_buf);
					...
					if ((p_item->flag2 & SESSION_FLAG2_CPU_IN) && !ppa_bypass_lro(ppa_buf)) {
						if ((ppa_add_lro_entry(p_item, 0)) == PPA_SUCCESS) {
							ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"lro entry added\n");
						} else {
							ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"lro entry add failed\n");
				...
				if (p_item->flags & SESSION_LAN_ENTRY) {
					...
					if (!PKTPRS_IS_ESP(desc->rx, PKTPRS_HDR_LEVEL0) &&
						!PKTPRS_IS_ESP(desc->rx, PKTPRS_HDR_LEVEL1)) {
				...
				/* Store the src mac from ingress descriptor.
				 * Needed for routed sessions.
				 */
				pktprs_src_mac(p_item->s_mac, desc->rx);
				/* Store egress src and dst mac address from the
				 * pktprs. This is needed for bridged case.
				 */
...
#if (IS_ENABLED(CONFIG_INTEL_VPN) || IS_ENABLED(CONFIG_MXL_VPN)) && (IS_ENABLED(CONFIG_X86_INTEL_LGM) || IS_ENABLED(CONFIG_SOC_LGM))
/**
...
#endif /* (CONFIG_INTEL_VPN || CONFIG_MXL_VPN) && (CONFIG_X86_INTEL_LGM || CONFIG_SOC_LGM) */
...
void ppa_api_session_manager_exit(void)
{
	...
	if (g_timer_workq) {
		cancel_delayed_work_sync(&check_hit_stat_dwork);
...
#if IS_ENABLED(CONFIG_X86_INTEL_LGM) || IS_ENABLED(CONFIG_SOC_LGM)
	if (ppa_init_session_item_max()) {
		ppa_debug(DBG_ENABLE_MASK_ERR,"Failed to initialize max item\n");
		goto PPA_API_SESSION_MANAGER_CREATE_FAIL;
	}
#endif
	reset_local_mib();
