...
#define DEFAULT_SESSION_TIMEOUT_IN_SEC		60	/*	1 minute*/
#define DEFAULT_NON_ACCEL_SESSION_TIMEOUT_IN_SEC 5	/*	5 seconds*/
...
#define SESSION_FLAG2_RXCSUM                    0x00020000
...
struct uc_session_node {
	...
	uint32_t		ct_timeout;		/* conntrack timeout in jiffie */
	...
	uint32_t		last_hit_time;		/* last hit time in seconds; Updated by bookkeeping thread */
	...
	uint32_t 		last_hit_time_jif;	/* last hit time in jiffies; Updated by bookkeeping thread */
	int32_t			dest_qid;		/* dest queue id */
#endif
#if IS_ENABLED(CONFIG_SOC_GRX500)
...
#endif
	uint8_t			s_mac[PPA_ETH_ALEN];	/* source mac of ingress packet for macdb
							 * learning
							 */
...
typedef struct {
	...
	uint32_t		priority;			/*skb priority*/
	union {
		uint32_t	extmark;			/*skb extmark value */
		uint32_t	mark;				/*skb mark value */
	} qos_mark;
} MC_GROUP_INFO;
...
#if IS_ENABLED(CONFIG_X86_INTEL_LGM) || IS_ENABLED(CONFIG_SOC_LGM)
typedef struct {
	uint32_t		delay_avg;		/* Avg delay between packets recived on this dst port */
	uint32_t		last_hit_time;		/* For calculating delay_avg */
	int			tx_ifindex;		/* ifindex of the tx netif */
	uint16_t		dport;			/* RTP port number */
	uint8_t			hit_count;		/* How many packets have been hit for this stream */
} MC_RTP_INFO;
#endif /*IS_ENABLED(CONFIG_X86_INTEL_LGM) || IS_ENABLED(CONFIG_SOC_LGM)*/
...
int32_t __ppa_lookup_mc_gid(IP_ADDR_C *, uint16_t, struct mc_session_node **);
...
extern uint32_t g_non_accel_session_timeout;
