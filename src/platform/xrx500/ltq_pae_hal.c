...
#if defined(PPA_API_PROC)
static struct dentry *dbgfs_ppa_pae;
static struct dentry *dbgfs_ppa_lro;
...
#if defined(PPA_API_PROC)
static const struct seq_operations dbgfs_read_pae_route_seq_ops = {
...
static const struct seq_operations dbgfs_read_pae_pce_seq_ops = {
...
static const struct file_operations dbgfs_file_pae_route_seq_fops = {
	.owner		= THIS_MODULE,
...
static const struct file_operations dbgfs_file_pae_pce_seq_fops = {
	.owner		= THIS_MODULE,
...
static const struct file_operations dbgfs_file_pae_rtstats_seq_fops = {
	.owner		= THIS_MODULE,
...
static const struct file_operations dbgfs_file_pae_accel_seq_fops = {
	.owner		= THIS_MODULE,
...
static const struct file_operations dbgfs_file_pae_debug_seq_fops = {
	.owner		= THIS_MODULE,
...
#ifdef PAE_HW_METER
static const struct file_operations dbgfs_file_pae_meter_seq_fops = {
	.owner		= THIS_MODULE,
...
static const struct file_operations dbgfs_file_pae_lrolen_seq_fops = {
	.owner		= THIS_MODULE,
...
static const struct file_operations dbgfs_file_pae_lroflush_seq_fops = {
	.owner		= THIS_MODULE,
...
static const struct file_operations dbgfs_file_pae_lrotimeout_seq_fops = {
	.owner		= THIS_MODULE,
...
{
	return seq_open(file, &dbgfs_read_pae_route_seq_ops);
}
...
{
	return seq_open(file, &dbgfs_read_pae_pce_seq_ops);
}
...
static struct ppa_debugfs_files pae_hal_debugfs_files[] = {
	{ "route",    0600, &dbgfs_file_pae_route_seq_fops },
	{ "pce",      0600, &dbgfs_file_pae_pce_seq_fops },
	{ "rtstats",  0600, &dbgfs_file_pae_rtstats_seq_fops },
	{ "accel",    0600, &dbgfs_file_pae_accel_seq_fops },
	{ "dbg",      0600, &dbgfs_file_pae_debug_seq_fops },
#ifdef PAE_HW_METER
	{ "hw_meter", 0600, &dbgfs_file_pae_meter_seq_fops },
#endif
};

static struct ppa_debugfs_files lro_hal_debugfs_files[] = {
	{ "pkt_len", 0600, &dbgfs_file_pae_lrolen_seq_fops },
	{ "flush",   0600, &dbgfs_file_pae_lroflush_seq_fops },
	{ "timeout", 0600, &dbgfs_file_pae_lrotimeout_seq_fops },
};

void pae_dbgfs_file_create(void)
{
	ppa_debugfs_create(ppa_hal_debugfs_dir_get(), "pae",
		&dbgfs_ppa_pae, pae_hal_debugfs_files,
		ARRAY_SIZE(pae_hal_debugfs_files));
	ppa_debugfs_create(ppa_hal_debugfs_dir_get(), "lro",
		&dbgfs_ppa_lro, lro_hal_debugfs_files,
		ARRAY_SIZE(lro_hal_debugfs_files));
}

void pae_dbgfs_file_remove(void)
{
	ppa_debugfs_remove(dbgfs_ppa_lro,
		lro_hal_debugfs_files,
		ARRAY_SIZE(lro_hal_debugfs_files));
	ppa_debugfs_remove(dbgfs_ppa_pae,
		pae_hal_debugfs_files,
		ARRAY_SIZE(pae_hal_debugfs_files));
}
...
int32_t add_routing_entry(PPA_ROUTING_INFO *route)
{
	...
	if (ret < GSW_statusOk) {
		...
		PPA_HAL_STATS_INC(rtstats, uc_dropped_sess);
		switch(ret) {
		...
		if(rt_entry.nRtIndex >=0 ) {	
			...
			nsess_add_succ++;
			if (p_item->flags & SESSION_IS_IPV6)
				PPA_HAL_STATS_INC(rtstats, curr_uc_ipv6_session);
			else
				PPA_HAL_STATS_INC(rtstats, curr_uc_ipv4_session);
		} else {
#ifdef PAE_HW_METER
			ppa_pae_meter_lock();
			put_session_nMeterId(p_item);
...
int32_t del_routing_entry(PPA_ROUTING_INFO *route)
{
	...
	if (ret < GSW_statusOk) {
		...
		nsess_del++;
		if (p_item->flags & SESSION_IS_IPV6)
			PPA_HAL_STATS_DEC(rtstats, curr_uc_ipv6_session);
		else
			PPA_HAL_STATS_DEC(rtstats, curr_uc_ipv4_session);
#ifdef PAE_HW_METER
		ppa_pae_meter_lock();
		put_session_nMeterId(p_item);
		ppa_pae_meter_unlock();
...
int32_t add_wan_mc_entry(PPA_MC_INFO *mc_route)
{
	...
	if(p_item->grp.sess_info->ip_mc_group.f_ipv6) {
		rt_entry.routeEntry.pattern.eIpType =	GSW_RT_IP_V6;
		rt_entry.routeEntry.action.eIpType =	GSW_RT_IP_V6;
		ppa_memcpy(rt_entry.routeEntry.pattern.nSrcIP.nIPv6,p_item->grp.sess_info->source_ip.ip.ip6,sizeof(uint16_t)*8);
		ppa_memcpy(rt_entry.routeEntry.pattern.nDstIP.nIPv6,p_item->grp.sess_info->ip_mc_group.ip.ip6,sizeof(uint16_t)*8);
		/* Derive multicast MAC from the multicast IP */
		ipv6_eth_mc_map((struct in6_addr *)p_item->grp.sess_info->ip_mc_group.ip.ip6, mc_dst_mac);
	} else {
		rt_entry.routeEntry.pattern.eIpType =	GSW_RT_IP_V4;	
		rt_entry.routeEntry.action.eIpType =	GSW_RT_IP_V4;
		if (is_ip_zero(&p_item->grp.sess_info->source_ip)) {
			rt_entry.routeEntry.pattern.nSrcIP.nIPv4 = PSEUDO_MC_ANY_SRC;
		} else {
			rt_entry.routeEntry.pattern.nSrcIP.nIPv4 = p_item->grp.sess_info->source_ip.ip.ip;
		}
		rt_entry.routeEntry.pattern.nDstIP.nIPv4 = p_item->grp.sess_info->ip_mc_group.ip.ip;
		/* Derive multicast MAC from the multicast IP */
		ip_eth_mc_map(p_item->grp.sess_info->ip_mc_group.ip.ip, mc_dst_mac);
	}
	...
	if (p_item->num_vap <= 1) {
		...
		if (!(p_item->flag2 & SESSION_FLAG2_BRIDGED_SESSION)) {
			...
			rt_entry.routeEntry.action.eSessRoutingMode = 1;
		}
	}
	...
	GSWR_OPS(pae_ops, ROUTE_SessionEntryAdd, &rt_entry, ret);
	if (ret < GSW_statusOk) {
		PPA_HAL_STATS_INC(rtstats, mc_dropped_sess);
		if( ret == GSW_statusLock_Failed) {
	...
	if (p_item->flags & SESSION_IS_IPV6)
		PPA_HAL_STATS_INC(rtstats, curr_mc_ipv6_session);
	else
		PPA_HAL_STATS_INC(rtstats, curr_mc_ipv4_session);
	return PPA_SUCCESS;
}
...
int32_t del_wan_mc_entry(PPA_MC_INFO *mc_route)
{
	...
	if (ret < GSW_statusOk) {
		...
		nsess_del++;
		if (p_item->grp.sess_info->ip_mc_group.f_ipv6)
			PPA_HAL_STATS_DEC(rtstats, curr_mc_ipv6_session);
		else
			PPA_HAL_STATS_DEC(rtstats, curr_mc_ipv4_session);
	}
	return ret;	
}
...
static void get_hal_stats(PPA_HAL_STATS *stat)
{
	...
	stat->max_uc_session = MAX_ROUTING_ENTRIES;
	stat->max_mc_session = MAX_WAN_MC_ENTRIES;
...
static int32_t lro_hal_generic_hook(PPA_GENERIC_HOOK_CMD cmd, void *buffer, uint32_t flag)
{
	switch (cmd) {
	...
	default:
		dbg("Command not supported:0x%0x\n", cmd);
		return PPA_FAILURE;
	}
}
...
static inline void hal_init(void)
{
	...
	ppa_drv_generic_hal_register(LRO_HAL, lro_hal_generic_hook);
